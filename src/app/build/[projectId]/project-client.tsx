...
export default function FactoryProjectClient({
  initialProject = null,
}: {
  initialProject?: FactoryProject | null;
}) {
  const params = useParams<{ projectId: string }>();
  const searchParams = useSearchParams();
  const id = params.projectId;
  const [project, setProject] = useState<FactoryProject | null>(initialProject);
  const [pipeline, setPipeline] = useState(
    getPipelineSteps(initialProject?.pipelineVersion ?? "v3")
  );
  ...
  useEffect(() => {
    ...
    const failSafe = window.setTimeout(() => {
      ...
      setError((e) => e || "PROJECT NOT FOUND");
    }, 6_000);
  ...
  if (loadState === "missing") {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center">
        <p className="text-lg font-semibold text-rose-400">PROJECT NOT FOUND</p>
        <p className="mt-2 text-sm text-zinc-500">
          {error && error !== "PROJECT NOT FOUND" ? error : null}
        </p>
        <p className="mt-2 text-sm text-zinc-500">
          This project is not in Supabase for this URL. Create a new one from
          /build.
        </p>
    ...
    return (
      <div className="mx-auto flex min-h-[50vh] w-full max-w-full flex-col items-center justify-center px-4 text-center">
        ...
        <p className="mt-3 max-w-sm text-xs text-zinc-600">
          If this stays here, the project is missing — you will see PROJECT NOT
          FOUND.
        </p>
      </div>
    );
      ...
      <div className="flex min-w-0 flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button size="sm" asChild>
            <Link href={`/preview/${id}`}>Open app</Link>
          </Button>
          ...
          <CardHeader>
            <CardTitle className="flex flex-wrap items-center justify-between gap-2">
              ...
              <Button size="sm" variant="secondary" asChild>
                <Link href={`/preview/${id}`}>Open full preview</Link>
              </Button>
            </CardTitle>
          </CardHeader>
          ...
          <CardContent className="space-y-4 text-sm">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <p className="break-all text-zinc-200">
                  {project.sandbox.previewUrl || `/preview/${id}`}
                </p>
              </div>
              <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button size="sm" variant="outline" asChild>
                <Link href={`/preview/${id}`}>Open preview</Link>
              </Button>
            </div>
        ...
        <Card className="mt-6 border-emerald-500/30">
          <CardContent className="p-6 sm:p-8">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              <Button asChild>
                <Link href={project.sandbox.productionUrl || `/preview/${id}`}>
                  Open business
                </Link>
              </Button>
